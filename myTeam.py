...
class ClassicPlanAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    enemyScared = 0
    for enemy in self.enemies:
      if gameState.data.agentStates[enemy].scaredTimer > 8:
        enemyScared = 1
    if enemyScared:
      ...
      bestActions = [a for a, v in zip(actions, values) if v == maxValue]
      toAct = random.choice(bestActions)
      print('=== 473 ===', self.index, toAct, timeLeft, myPos)
      return toAct
  ...
  def getFeaturesDeepFood(self, gameState, action):
    ...
    if len(deepFoodDict.keys()) > 0: # This should always be True,  but better safe than sorry
      minDistance = min([self.getFoodDistance(nextPos, food, gameState) for food in deepFoodDict.keys()])
